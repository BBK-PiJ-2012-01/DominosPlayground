 ...
 * Time: 17:50
 */
public class AIPlayerRefactored extends ObservantPlayer {
    ...
    @Override
    public Play makeEducatedPlay(Table table, List<Choice> opponentsLastChoices) throws CantPlayException {
        if (isFirstMove()) {
            boolean myGoFirst = opponentsLastChoices.isEmpty();
            ImmutableBone[] initialLayout = Bones.convertToImmutableBoneList(getInitialLayout()).toArray(new ImmutableBone[0]);
            ai.setInitialState(initialHand, myGoFirst, initialLayout);
        }
    ...
    @Override
    public void makeEducatedDraw(BoneYard boneYard, boolean initialPickup) {
